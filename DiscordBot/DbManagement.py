...
def QualifiedQuote(Quote: str) -> bool:
    return (re.search("<a>", Quote) is not None) and (
        re.search("<v>", Quote) is not None
    )
...
def ReadSnipes(NumSnipes: int = 5) -> list:
    ...
    try:
        ...
        cur = conn.cursor()
        data = cur.execute(
            f"SELECT * FROM {SNIPES_T} WHERE IsDeleted=0 ORDER BY Timestamp DESC LIMIT {NumSnipes};"
        ).fetchall()
...
def CountSniperSnipedAmount(SniperId: int, SnipedId: int) -> int:
    ...
    try:
        conn = sqlite3.connect(CONNECTION_PATH)
        sql = (
            f"SELECT * FROM {SNIPES_T} WHERE IsDeleted=0 AND SniperId=? AND SnipedId=?;"
        )
        param = (
            SniperId,
            SnipedId,
        ...
        print(
            f"File: {FILE_NAME} -- Funct: CountSniperSnipedAmount -- Error: {str(ex)}"
        )
        result = -1
    finally:
        conn.close()
